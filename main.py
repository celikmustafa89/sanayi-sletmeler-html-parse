    ...
    try:
        ...
        if len(tds) >= 5:
            for i in range(0, len(tds), 5):
                info_dict[' '.join(tds[i + 1].get_text().split()) + " - " + str(int((i / 5) + 1))] = ' '.join(
                    tds[i + 2].get_text().split())
                header_counter = add_header_dict(
                    ' '.join(tds[i + 1].get_text().split()) + " - " + str(int((i / 5) + 1)), header_counter)
                print("{}: {}".format(' '.join(tds[i + 1].get_text().split()), ' '.join(tds[i + 2].get_text().split())))

                info_dict[' '.join(tds[i + 3].get_text().split()) + " - " + str(int((i / 5) + 1))] = ' '.join(
                    tds[i + 4].get_text().split())
                header_counter = add_header_dict(
                    ' '.join(tds[i + 3].get_text().split()) + " - " + str(int((i / 5) + 1)), header_counter)
